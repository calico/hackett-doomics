...
# other figure-specific configuration

# drive ID for manuscript figures and other assets
manuscript_drive_root <- googledrive::as_id(params$manuscript_drive_root)

# load SHOCK results from Drive
# its unclear whether and how much of this data we can release so
# we need to make it very explicit when we load these results
source(file.path(params$repo_path, "utils", "data_prep.R"))
mouse_phenotypes <- load_mouse_phenotypes(shock_token = params$shock_token)
```
...
path_out_base_dir <- file.path(load_domics("informatics", domics_paths), "metabolites_lipids")
...
injection_plots_df <- featurization_parameters %>%
  ...
  mutate(
    plot_path = file.path(dataset_path, "out_figures", "injection_comparison-1.png"),
    exists = file.exists(plot_path),
    updated_name = glue::glue("{data_type}_injection_comparison.png"),
    updated_path = file.path(params$figures_dir, updated_name)
    ) 
...
all_normalized_peaks <- featurization_parameters %>%
  mutate(
    peaks_path = file.path(dataset_path, "out_files", "normalized_small_molecules.Rds"),
...
```{r proteomics_plots}
proteomics_informatics_plot_path <- file.path(load_domics("informatics", domics_paths), "proteomics", "out_figures")
