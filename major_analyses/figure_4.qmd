...
After assigning pathways to specific modes of aging we can select some of the strongest and/or most interesting features associated with each pathway to add context to univariate volcano plots, and bivariate signed-significance plots.
...
- figure_4_out:
    ...
    - term_affinity_summaries: pathways assigned to specific aging modes
...
feature_signif <- figure_3_out$feature_signif_aug
...
aging_ex <- plot_aging(features_with_design, feature_signif, feature_names = feature_subset, facet_ncol = 6)$plots$aging + theme(legend.position = "none") + labs(title = NULL) + theme(legend.position = "none")
age_x_lifespan_ex <- plot_age_by_lifespan(features_with_design, feature_signif, feature_names = feature_subset, facet_ncol = 6)$plots$age_by_lifespan + labs(title = NULL) + theme(legend.position = "none")
fll_ex <- plot_fraction_of_life_lived(features_with_design, feature_signif, feature_names = feature_subset, facet_ncol = 6)$plots$fraction_of_life_lived + labs(title = NULL)
