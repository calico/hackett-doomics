...
params <- list(
  ...
  do_drive_token = Sys.genev("doomics_drive_working_uri"),
...
full_dataset_path <- file.path(params$project_path, params$dataset_path)

if (params$inclusion_list == "") {
  full_inclusion_file_path = NA_character_
} else {
  full_inclusion_file_path = file.path(params$repo_path, "supporting_files",
                                       "inclusion_lists", params$inclusion_list)
}
...
if (params$annotation_data_type %in% c("lipids-neg", "lipids-pos")) {
...
small_molecules <- claman::floor_peaks(small_molecules, params$log2_limit)
```
...
bookmark_eic_comparison <- small_molecules$features %>%
  ...
  dplyr::mutate(adductName = ifelse(
    is.na(adductName),
    ifelse(stringr::str_detect(params$annotation_data_type, "neg"), "[M-H]-", "[M+H]+"),
    adductName
    )) %>%
...
if (params$set != "3") {
  ...
  design_matrix <- read_design_matrix(file.path(params$repo_path, "supporting_files",
...
zero_counts <- samples_versus_negctl_df %>%
  ...
  dplyr::mutate(above_limit = log2_abundance > params$log2_limit + 0.001,
...
dataset_nonmissing_sample_fractions <- tall_preliminary %>%
  ...
  dplyr::group_by(groupId) %>%
  dplyr::summarize(nonmissing_frac = sum(log2_abundance > params$log2_limit + 0.001)/n())
...
loading_controlled <- claman::normalize_peaks(
  ...
  log2_floor_value = params$log2_limit
  )
...
posctl_summary <- normalized_peaks %>%
  ...
  dplyr::filter(log2_abundance > params$log2_limit + 0.001) %>%
...
# choose normalized measure based on params$normalization_var
  
normalization_selected_measurements <- normalized_peaks$measurements %>%
  # choose normalized measure based on params$normalization_var
  dplyr::mutate(normalized_log2_abundance = !!rlang::sym(params$normalization_var))
...
sample_type_measures <- valid_mzroll_list$measurements %>%
  dplyr::filter(normalized_log2_abundance > params$log2_limit + 0.001) %>%
  dplyr::left_join(
...
saveRDS(bio_small_molecules, file = file.path(params$run_outdir, "out_files", "small_molecules.Rds"))
# all reduced biological samples and biologically enriched peakgroups
saveRDS(normalized_peaks, file = file.path(params$run_outdir, "out_files", "normalized_small_molecules.Rds"))
# final reduced biological samples and valid peakgroups
saveRDS(valid_mzroll_list, file = file.path(params$run_outdir, "out_files", "valid_small_molecules.Rds"))
```
